 ...
 */
public abstract class MqlNode implements Cloneable {
    ...
    /**
     * @deprecated Please use {@link #clone(MqlNode)}; this method brings along too much baggage from early versions of Java
     */
    @Override
    @Deprecated
     ...
     * @param leftPrec The precedence of the {@link MqlNode} immediately preceding this node in a depth-first scan of the parse tree
     * @param rightPrec The precedence of the {@link MqlNode} immediately
     */
    ...
    public void validateExpr( SqlValidator validator, SqlValidatorScope scope ) {
        ...
        // Util.discard( validator.deriveType( scope, this ) ); // TODO DL: disabled
    }
    ...
    /**
     * Returns whether two nodes are equal (using {@link #equalsDeep(MqlNode, Litmus)}) or are both null.
     *
    ...
    public static boolean equalDeep( List<MqlNode> operands0, List<MqlNode> operands1, Litmus litmus ) {
        ...
        for ( int i = 0; i < operands0.size(); i++ ) {
            if ( !MqlNode.equalDeep( operands0.get( i ), operands1.get( i ), litmus ) ) {
                return litmus.fail( null );
            }
        }
